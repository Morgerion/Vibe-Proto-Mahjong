...
function M.load()
	...
	local regular_data = assert(sys.load_resource("/fonts/Montserrat-Regular.ttf"))
	fonts["Regular"] = imgui.font_add_ttf_data(regular_data, #regular_data, fontsize, fontsizebase, imgui.GLYPH_RANGES_CYRILLIC)
	local bold_data = assert(sys.load_resource("/fonts/Montserrat-Bold.ttf"))
	fonts["Bold"] = imgui.font_add_ttf_data(bold_data, #bold_data, fontsize, fontsizebase)
	local italic_data = assert(sys.load_resource("/fonts/Montserrat-Italic.ttf"))
	fonts["Italic"] = imgui.font_add_ttf_data(italic_data, #italic_data, fontsize, fontsizebase)
	local bolditalic_data = assert(sys.load_resource("/fonts/Montserrat-BoldItalic.ttf"))
